                ...
                {oltPorts.map((p) => {
                  ...
                  return (
                    <tr key={p.id}>
                      ...
                      <td>{(p.latitude && p.longitude) ? (<a href={`https://maps.google.com/?q=${p.latitude},${p.longitude}`} target="_blank" rel="noopener" className="btn btn-sm btn-outline-primary"><i className="bi bi-map"></i></a>) : <span className="text-muted small">—</span>}</td>
                      ...
                      <td className="text-end"><div className="btn-group btn-group-sm"><button className="btn btn-outline-success" onClick={() => onAddNode(p.id)} title="Add ODC"><i className="bi bi-diagram-2"></i></button><button className="btn btn-outline-danger" onClick={() => onDelete(p.id)} title="Delete"><i className="bi bi-trash"></i></button></div></td>
                    </tr>
                  );
                })}
              </tbody>
