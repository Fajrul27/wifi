...
import { createCustomIcon, MARKER_CONFIG, isValidCoord } from "../admin/utils/mapUtils";
...
export default function TechnicianDashboard({
  ...
  const [apiSelectedRouter_local, setApiSelectedRouter_local] = useState(null);
  ...
  const connections = useMemo(() => {
    ...
    return lines;
  }, [filteredOltPorts, filteredNodes, filteredUsers, hasOnlineUser, hasAnyUser, routers, showLines, shouldShowClients, isDeferredReady, nodes, nodesMap]);
  ...
  const renderGroupedEntityPopup = useCallback((group, index) => {
    ...
    return (
      <Popup key={popupKey} minWidth={450} autoPan={false} className={`custom-detail-popup two-column-popup ${isDarkMode ? 'dark-popup' : ''}`} eventHandlers={{ remove: () => { setActivePopup(null); setActiveGroupDetailNode(null); } }}>
          {isOpen ? (
  ...
  const renderGroupedClientPopup = useCallback((group, index) => {
    ...
    return (
      <Popup key={popupKey} minWidth={450} autoPan={false} className={`custom-detail-popup two-column-popup ${isDarkMode ? 'dark-popup' : ''}`} eventHandlers={{ remove: () => { setActivePopup(null); setActiveGroupDetailClient(null); } }}>
          {isOpen ? (
              <div className="d-flex" style={{ width: '550px' }}>
  ...
  }, [isDarkMode, setActivePopup, setLightbox, activePopup]);
